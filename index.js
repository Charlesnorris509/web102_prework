...
// Calculate total raised (this was missing in the original)
...
function filterUnfundedOnly() {
    ...
    const NonFundedGames = GAMES_JSON.filter((game) => game.pledged < game.goal)
...
const displayStr = `A total of $${totalAmountRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} ${GAMES_JSON.length > 1 ? "games." : "game."} Currently ${unfundedGamesCount} ${unfundedGamesCount > 1 ? "games" : "games"} remain unfunded. We need your help to fund these amazing games!`
...
let firstGamePledge = document.createElement("p");
firstGamePledge.innerHTML = first.name;
firstGameContainer.appendChild(firstGamePledge);

// do the same for the runner up item
const secondGameContainer = document.getElementById("second-game");
let secondGamePledge = document.createElement("p");
secondGamePledge.innerHTML = second.name;
secondGameContainer.appendChild(secondGamePledge);